...
import "../interfaces/common/IFullToken.sol";

// FDAO with Governance.
contract FDAO is ERC20("FDAO", "FDAO"), Ownable {
  ...
  IFullToken public full;

  constructor(IFullToken _full) public {
    full = _full;
  }

  // Safe full transfer function, just in case if rounding error causes pool to not have enough FULLs.
  function safeFullTransfer(address _to, uint256 _amount) public onlyOwner {
  ...
  function _delegate(address delegator, address delegatee) internal {
    ...
    uint256 delegatorBalance = balanceOf(delegator); // balance of underlying FDAOs (not scaled);
