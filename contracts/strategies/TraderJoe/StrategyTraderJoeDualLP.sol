...
contract StrategyTraderJoeDualLP is StratManager, FeeManager {
    ...
    constructor(
        ...
        address _keeper,
        address _strategist,
        address _fullDAOFeeRecipient,
        address[] memory _outputToNativeRoute,
        address[] memory _nativeToLp0Route,
        address[] memory _nativeToLp1Route
    ) StratManager(_keeper, _strategist, _unirouter, _vault, _fullDAOFeeRecipient) public {
        want = _want;
        poolId = _poolId;
        chef = _chef;
    ...
    function chargeFees(address callFeeRecipient) internal returns (uint256) {
        ...
        uint256 fullDAOFeeAmount = nativeBal.mul(fullDAOFee).div(MAX_FEE);
        IERC20(native).safeTransfer(fullDAOFeeRecipient, fullDAOFeeAmount);
    ...
    function rewardsAvailable() public view returns (uint256, uint256) {
        (uint256 outputBal,,,uint256 nativeBal) = IMasterChef(chef).pendingTokens(poolId, address(this));
